...
async function retryWithDelay<T>(
  ...
  try {
    ...
    if (retries > 0) {
      logger.warn(
        `Retrying operation in ${delay}ms. Retries left: ${retries - 1}`,
      );
...
async function fetchFromAWS(): Promise<ScrapedOrder[]> {
  ...
  try {
    ...
    return response.data.map((item: AWSApiItem): ScrapedOrder => {
      ...
      const detectedCategories = item.content
        ? determineCategories(item.content)
        : [];
      const detectedAgencies = item.content
        ? determineAgencies(item.content)
        : [];
...
export async function scrapeDocuments(): Promise<ScraperResult> {
  try {
    ...
    const existingLinks = new Set(
      existingOrders.map((o) => o.link).filter(Boolean),
    );
    const existingNumbers = new Set(
      existingOrders.map((o) => o.number).filter(Boolean),
    );
