...
async function main() {
  ...
  try {
    ...
    const executiveOrders = await prisma.$queryRaw`
      SELECT * FROM "ExecutiveOrder"
    `
    console.log(`Found ${(executiveOrders as any[]).length} executive orders`)
    ...
    // Get the status ID
    const [statusRecord] = await prisma.$queryRaw`
    ...
    for (const order of executiveOrders as any[]) {
