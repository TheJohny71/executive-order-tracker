...
const saveOrdersToDynamoDB = async (orders) => {
    const chunks = Array.from({ length: Math.ceil(orders.length / 25) }, (_, i) => orders.slice(i * 25, (i + 1) * 25));
    ...
    try {
        ...
        const initialOrders = await page.evaluate(() => {
            ...
            return articles.map(article => {
                ...
                const numberMatch = titleElement?.textContent?.match(/(?:Executive Order|Presidential Memorandum) (\d+)/);
                return {
        ...
        const detailedOrders = await Promise.all(initialOrders.map(async (order) => {
            try {
