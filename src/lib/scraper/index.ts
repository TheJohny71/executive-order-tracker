...
async function retryWithDelay<T>(
  ...
  try {
    ...
    if (retries > 0) {
      logger.warn(
        `Retrying operation in ${RETRY_DELAY}ms. Retries left: ${retries - 1}`,
      );
...
export async function scrapeDocuments(): Promise<ScraperResult> {
  try {
    ...
    logger.info(
      `Processing ${newOrders.length} new orders and ${updatedOrders.length} updates`,
    );
    ...
    for (const order of updatedOrders) {
      ...
      await prisma.order.updateMany({
        where: {
          OR: [{ link: order.url }, { number: order.identifier }],
        },
        data: {
          title: order.title,
          summary: order.summary,
